...
class SummaryDialog(qtw.QDialog):
    def __init__(self, parent=None, data=None, filteredIds=None, *args, **kwargs) -> None:
        ...
        self.ui.summaryData_searchBar = CustomLineEdit(
        ...
        self.ui.dataTreeSearchBox_widget.layout().insertWidget(
            0, self.ui.summaryData_searchBar)
    ...
    def populateSummaryData(self):
        for testcase, teststeps in self.testcaseData.items():
            ...
            for i in range(6):
                testcaseItem.setForeground(i, qtg.QColor('white'))
                testcaseItem.setBackground(i, qtg.QColor('darkGrey'))
            ...
            testcaseItem.setText(
                5, f"{testcaseItem.childCount()} / {testcase.teststepsCount}")
    ...
    def handleSearchBar(self, text):
        ...
        testcaseList = [summaryTree.topLevelItem(
            i) for i in range(summaryTree.topLevelItemCount())]
        ...
        for testcase in testcaseList:
            ...
            for i in range(testcase.childCount()):
                ...
                oldDescription = testcase.child(i).text(3)
            ...
            testcase.setHidden(
                False) if isAnyFound else testcase.setHidden(True)
